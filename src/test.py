...
class TestHci(unittest.TestCase):
    ...
    def test_dtm(self):
        ...
        self.assertEqual(hci1.tx_test_vs(num_packets=NUM_PACKETS),pc.StatusCode.LL_SUCCESS)
        ...
        self.assertEqual(rx_ok, stats_periph['rx-data'])
        self.assertEqual(NUM_PACKETS, stats_central['tx-data'])
        ...
        for key in stats_central:
            self.assertEqual(stats_central[key], 0)
    ...
    def test_commands(self):
        ...
        self.assertEqual(
            hci1.get_channel_map_periodic_scan_adv(1, False)[1], pc.StatusCode.LL_SUCCESS
        )
    ...
    def test_sniffer(self):
        ...
        if status == pc.StatusCode.LL_SUCCESS:
            self.assertEqual(
                hci1.enable_sniffer_packet_forwarding(False), pc.StatusCode.LL_SUCCESS
            )
