...
class TestHci(unittest.TestCase):
    ...
    def test_encryption(self):
        ...
        def pub_key_read_callback(packet: EventPacket):
            xcord, ycord = packet.get_return_params([8, 8])

            try:
                _ = Point(xcord, ycord, curve=P256)
            except ValueError:
                self.assertTrue(False, "Point not on curve. Encryption fail")

        hci1.read_local_p256_pub_key(callback=pub_key_read_callback)
        ...
        xcoord_bad = le_list_to_int(bad_key[:32])
